...
app.get('/api/activities/:id/registration-form', (req, res) => {
  ...
  res.json({ success: true, data: activity.registrationForm });
});
...
app.get('/api/activities/:id/check-eligibility', (req, res) => {
  ...
  res.json({ success: true, eligible: true, message: 'Eligible to register' });
});
...
app.post('/api/activities/:id/register', (req, res) => {
  ...
  const existingRegistration = findRegistrationByActivityAndStudent(activityId, studentId);
  if (existingRegistration) {
    return res.status(400).json({ success: false, message: 'Already registered for this activity' });
  }
...
app.put('/api/registrations/:id', (req, res) => {
  ...
  if (registration.status !== 'pending') {
    return res.status(400).json({ success: false, message: 'Cannot modify registration after approval' });
  }
...
app.delete('/api/registrations/:id', (req, res) => {
  ...
  if (registration.status !== 'pending') {
    return res.status(400).json({ success: false, message: 'Cannot cancel registration after approval' });
  }
